...
const demoData = {
  ...
  projects: [
    {
      pr_id: 1,
      pr_name: "Website Redesign",
      project_notes: "Complete overhaul of company website",
      status: "in_progress",
      completion: 65,
      dept_id: 1,
    ...
    {
      pr_id: 2,
      pr_name: "Mobile App v2",
      project_notes: "iOS and Android native apps",
      status: "in_progress",
      completion: 40,
      dept_id: 1,
    ...
    {
      pr_id: 3,
      pr_name: "Q2 Marketing Campaign",
      project_notes: "Social media and email marketing",
      status: "completed",
      completion: 100,
      dept_id: 2,
    ...
    {
      pr_id: 4,
      pr_name: "Brand Identity",
      project_notes: "New logo and brand guidelines",
      status: "on_hold",
      completion: 20,
      dept_id: 3,
  ...
  tasks: [
    ...
    },
    {
      ...
      task_note: "GitHub Actions config needed",
      assigned_to: 1,
      project_id: 1,
      due_date: "2026-03-01",
    },
  ...
  reports: [
    {
      rp_id: 1,
      st_id: 1,
      title: "Weekly Progress - Website",
      content:
        "Completed 3 major features this week. Homepage responsive design is 90% done.",
      date: "2026-06-09",
      read: false,
      type: "progress",
    },
    {
      rp_id: 2,
      st_id: 2,
      title: "Marketing Analytics Q2",
      content:
        "Campaign reached 50K impressions. CTR improved by 12% compared to Q1.",
      date: "2026-06-08",
      read: false,
      type: "analytics",
    },
    {
      ...
      content:
        "Found critical bug in authentication flow. Needs immediate attention.",
      date: "2026-06-07",
      read: true,
      type: "issue",
    },
    {
      ...
      content: "First draft of brand guidelines completed. Awaiting review.",
      date: "2026-06-06",
      read: true,
      type: "deliverable",
    },
  ...
  activities: [
    {
      ...
      user: "John Doe",
      action: "Updated task",
      details: "Homepage mockup marked complete",
      status: "success",
    },
    {
      time: "09:15 AM",
      ...
      details: "Marketing Analytics Q2",
      status: "info",
    },
    {
      time: "Yesterday",
      user: "Mike Johnson",
      action: "Created task",
      details: "API documentation task added",
      status: "success",
    },
    {
      time: "Yesterday",
      ...
      details: "Brand Identity 20% complete",
      status: "warning",
    },
  ],
};
...
async function loadDashboard() {
  ...
  document.getElementById("dash-projects").textContent = projects.filter(
    (p) => p.status === "in_progress",
  ).length;
  document.getElementById("dash-tasks").textContent = tasks.filter(
    (t) => !t.done,
  ).length;
  document.getElementById("dash-reports").textContent = reports.filter(
    (r) => !r.read,
  ).length;
...
function renderDeptPerformance(depts, staffs, projects) {
  ...
  depts.forEach((d) => {
    ...
    const avgCompletion = deptProjects.length
      ? Math.round(
          deptProjects.reduce((a, p) => a + p.completion, 0) /
            deptProjects.length,
        )
      : 0;
...
async function loadStaff() {
  ...
  filter.innerHTML =
    ...
    depts
      .map((d) => `<option value="${d.dp_id}">${d.dp_name}</option>`)
      .join("");

  if (!data.length) {
    tbody.innerHTML =
      '<tr><td colspan="8" class="empty-state">No staff members</td></tr>';
    return;
  }
...
function filterStaff() {
  ...
  if (visibleCount === 0 && deptId) {
    tbody.innerHTML =
      '<tr><td colspan="8" class="empty-state">No staff in this department</td></tr>';
  }
}
...
async function loadDepartments() {
  ...
  if (!data.length) {
    tbody.innerHTML =
      '<tr><td colspan="5" class="empty-state">No departments</td></tr>';
    return;
  }
...
async function loadJobs() {
  ...
  if (!data.length) {
    tbody.innerHTML =
      '<tr><td colspan="4" class="empty-state">No job positions</td></tr>';
    return;
  }
...
async function loadProjects() {
  ...
  // Stats
  document.getElementById("proj-completed").textContent = projects.filter(
    (p) => p.status === "completed",
  ).length;
  document.getElementById("proj-progress").textContent = projects.filter(
    (p) => p.status === "in_progress",
  ).length;
  document.getElementById("proj-hold").textContent = projects.filter(
    (p) => p.status === "on_hold",
  ).length;
  ...
  if (!projects.length) {
    grid.innerHTML =
      '<div class="empty-state" style="grid-column:1/-1;">No projects yet</div>';
    return;
  }
  ...
  projects.forEach((p) => {
    ...
    const projectStaff = records
      .filter((r) => r.pr_id == p.pr_id)
      .map((r) => staffs.find((s) => s.st_id == r.st_id))
      .filter(Boolean);
    const statusColors = {
      completed: "success",
      in_progress: "primary",
                ...
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <div class="project-team">
                        ${projectStaff
                          ...
                          .map(
                            (s) =>
                              `<div class="team-avatar">${s.st_name[0]}</div>`,
                          )
                          .join("")}
...
function viewProject(id) {
  ...
  const staffList = records
    .map((r) => demoData.staffs.find((s) => s.st_id == r.st_id))
    .filter(Boolean);
  ...
  const content = `
        ...
        <div style="margin-bottom:20px;">
            ...
            `,
                    ...
                    .join("")
                : '<div style="color:var(--secondary);padding:8px;">No staff assigned</div>'
            }
        </div>
        ...
        <div>
            ...
            `,
                    ...
                    .join("")
                : '<div style="color:var(--secondary);padding:8px;">No tasks</div>'
            }
        </div>
    `;
...
async function loadTasks() {
  ...
  data.forEach((row) => {
    ...
    const isOverdue =
      !row.done && row.due_date && new Date(row.due_date) < new Date();
...
function toggleTaskStatus(id) {
  ...
  addActivity(
    task.done ? "Completed task" : "Reopened task",
    task.task,
    "success",
  );
...
async function openModal(type, id = null) {
  ...
  document.getElementById("modal-title").textContent =
    (id ? "Edit" : "Add") + " " + currentFormConfig.title;
    ...
    if (field.type === "select") {
      ...
      if (field.options) {
        ...
        data.forEach((item) => {
          ...
          const lastName =
            field.showLastname && item.st_lastname
              ? " " + item.st_lastname
              : "";
...
async function saveForm() {
  ...
  currentFormConfig.fields.forEach((field) => {
    ...
    if (field.type === "select" && field.name === "done")
  ...
  const idField =
    currentFormConfig.table === "staffs"
      ? "st_id"
      : currentFormConfig.table === "departments"
        ? "dp_id"
        : currentFormConfig.table === "jobs"
          ? "jb_id"
          : currentFormConfig.table === "projects"
            ? "pr_id"
            : currentFormConfig.table === "tasks"
              ? "ts_id"
              : currentFormConfig.table === "reports"
                ? "rp_id"
                : currentFormConfig.table === "project_records"
                  ? "pjr_id"
                  : "id";
  ...
  const endpoint = editingId
    ? `${currentFormConfig.table}/${editingId}`
    : currentFormConfig.table;
  ...
  try {
    if (demoMode) {
      ...
      if (editingId) {
        const idx = demoData[table].findIndex(
          (item) => item[idField] == editingId,
        );
        if (idx !== -1)
          demoData[table][idx] = { ...demoData[table][idx], ...data };
        addActivity(
          `Updated ${currentFormConfig.title.toLowerCase()}`,
          data[currentFormConfig.fields[0].name],
          "success",
        );
      } else {
        const newId =
          Math.max(...demoData[table].map((item) => item[idField] || 0), 0) + 1;
        data[idField] = newId;
        demoData[table].push(data);
        addActivity(
          `Created ${currentFormConfig.title.toLowerCase()}`,
          data[currentFormConfig.fields[0].name],
          "success",
        );
      }
      showToast(
        `${currentFormConfig.title} ${editingId ? "updated" : "created"} successfully`,
        "success",
      );
    } else {
      if (editingId) {
      ...
      addActivity(
        `${editingId ? "Updated" : "Created"} ${currentFormConfig.title.toLowerCase()}`,
        data[currentFormConfig.fields[0].name],
        "success",
      );
      showToast(
...
async function deleteItem(table, id, itemName = "") {
  ...
  if (table === "departments") {
    ...
    const recordCount = demoData.project_records.filter(
      (r) => r.pr_id == id,
    ).length;
...
const API_BASE = "http://localhost:3000/api"; // <-- UPDATE THIS

// ==================== STATE ====================
let currentTable = "staffs";
let editingId = null;
let currentTab = "dashboard";
...
const demoData = {
  ...
  departments: [
    { dp_id: 1, dp_name: "Engineering" },
    { dp_id: 2, dp_name: "Marketing" },
  ],
  jobs: [
    { jb_id: 1, job: "Developer" },
    { jb_id: 2, job: "Manager" },
  ...
  projects: [
    {
      pr_id: 1,
      pr_name: "Website Redesign",
      project_notes: "Company website overhaul",
    },
    { pr_id: 2, pr_name: "Mobile App", project_notes: "iOS and Android app" },
  ],
  project_records: [
    {
      pjr_id: 1,
  ...
  daily_work_reports: [
    { st_id: 1, pjr_id: 1, date: "2024-01-15", work_hour: 8 },
  ],
  tasks: [
    { ts_id: 1, task: "Design homepage", done: 0, task_note: "Use Figma" },
    {
      ts_id: 2,
      task: "Setup database",
      done: 1,
      task_note: "MySQL configured",
    },
  ],
};
...
function showTab(tab) {
  ...
  currentTab = tab;
  if (tab !== "dashboard") loadTableData(tab);
  else loadDashboard();
}
...
async function apiPost(endpoint, data) {
  try {
    ...
    showToast("API not connected. Data saved locally.", "error");
    return { success: true };
  }
}
...
async function apiDelete(endpoint, id) {
  try {
    ...
    showToast("API not connected. Deleted locally.", "error");
    return { success: true };
  }
}
...
async function loadDashboard() {
  ...
  const projects = await apiGet("projects");
  const tasks = await apiGet("tasks");
  document.getElementById("stat-staffs").textContent = staffs.length;
  document.getElementById("stat-depts").textContent = depts.length;
  document.getElementById("stat-projects").textContent = projects.length;
  document.getElementById("stat-tasks").textContent = tasks.filter(
    (t) => !t.done,
  ).length;
}
...
async function loadTableData(table) {
  ...
  const data = await apiGet(currentTable);
  const tbody = document.querySelector(
    `#${table === "project_records" ? "records" : table === "daily_work" ? "work" : table}-table tbody`,
  );

  if (!data || data.length === 0) {
    tbody.innerHTML = `<tr><td colspan="10" style="text-align:center;color:#94a3b8;padding:40px">No data available</td></tr>`;
    return;
  }
  ...
  for (const row of data) {
    html += generateRowHtml(table, row, {
      depts,
      jobs,
      staffs,
      projects,
...
function generateRowHtml(table, row, lookups) {
  const { depts, jobs, staffs, projects, records } = lookups;
  ...
  switch (table) {
    case "staffs":
      ...
      return `<tr>
                ...
                <td class="actions">
                    <button class="action-btn edit" onclick="editItem('staff', ${row.st_id})">Edit</button>
                    <button class="action-btn delete" onclick="deleteItem('staffs', ${row.st_id})">Delete</button>
                </td>
            </tr>`;
    ...
    case "departments":
      return `<tr>
                ...
                <td class="actions">
                    <button class="action-btn edit" onclick="editItem('department', ${row.dp_id})">Edit</button>
                    <button class="action-btn delete" onclick="deleteItem('departments', ${row.dp_id})">Delete</button>
                </td>
            </tr>`;
    ...
    case "jobs":
      return `<tr>
                ...
                <td class="actions">
                    <button class="action-btn edit" onclick="editItem('job', ${row.jb_id})">Edit</button>
                    <button class="action-btn delete" onclick="deleteItem('jobs', ${row.jb_id})">Delete</button>
                </td>
            </tr>`;
    ...
    case "projects":
      return `<tr>
                ...
                <td class="actions">
                    <button class="action-btn edit" onclick="editItem('project', ${row.pr_id})">Edit</button>
                    <button class="action-btn delete" onclick="deleteItem('projects', ${row.pr_id})">Delete</button>
                </td>
            </tr>`;

    case "project_records":
      const staff = staffs.find((s) => s.st_id == row.st_id);
      const proj = projects.find((p) => p.pr_id == row.pr_id);
      return `<tr>
                <td>${row.pjr_id}</td>
                <td>${staff ? staff.st_name + " " + staff.st_lastname : "N/A"}</td>
                <td>${proj ? proj.pr_name : "N/A"}</td>
                <td>${row.start_time}</td>
                <td>${row.end_time || "Ongoing"}</td>
                <td class="actions">
                    <button class="action-btn edit" onclick="editItem('project_record', ${row.pjr_id})">Edit</button>
                    <button class="action-btn delete" onclick="deleteItem('project_records', ${row.pjr_id})">Delete</button>
                </td>
            </tr>`;

    case "daily_work":
      const wstaff = staffs.find((s) => s.st_id == row.st_id);
      const wrec = records.find((r) => r.pjr_id == row.pjr_id);
      const wproj = wrec ? projects.find((p) => p.pr_id == wrec.pr_id) : null;
      return `<tr>
                <td>${wstaff ? wstaff.st_name : "N/A"}</td>
                <td>${wproj ? wproj.pr_name : "N/A"}</td>
                <td>${row.date}</td>
                <td>${row.work_hour} hrs</td>
                <td class="actions">
                    <button class="action-btn edit" onclick="editItem('daily_work', '${row.st_id}-${row.pjr_id}-${row.date}')">Edit</button>
                    <button class="action-btn delete" onclick="deleteItem('daily_work_reports', '${row.st_id}-${row.pjr_id}-${row.date}')">Delete</button>
                </td>
            </tr>`;
    ...
    case "tasks":
      return `<tr>
                <td>${row.ts_id}</td>
                <td>${row.task}</td>
                <td><span class="badge ${row.done ? "badge-green" : "badge-red"}">${row.done ? "Done" : "Pending"}</span></td>
                <td>${row.task_note || "-"}</td>
                <td class="actions">
                    <button class="action-btn edit" onclick="editItem('task', ${row.ts_id})">Edit</button>
                    <button class="action-btn delete" onclick="deleteItem('tasks', ${row.ts_id})">Delete</button>
...
const formConfigs = {
  staff: {
    title: "Staff Member",
    fields: [
      { name: "st_name", label: "First Name", type: "text", required: true },
      { name: "st_lastname", label: "Last Name", type: "text", required: true },
  ...
  job: {
    title: "Job",
    fields: [{ name: "job", label: "Job Title", type: "text", required: true }],
  },
  project: {
    title: "Project",
    fields: [
      { name: "pr_name", label: "Project Name", type: "text", required: true },
      { name: "project_notes", label: "Notes", type: "textarea" },
    ],
  },
  project_record: {
    title: "Project Assignment",
  ...
  task: {
    ...
    fields: [
      ...
      { name: "task_note", label: "Note", type: "textarea" },
    ],
  },
};
  ...
  for (const field of config.fields) {
    ...
    if (field.type === "select" && field.source) {
      ...
      for (const item of data) {
        const selected =
          editData && editData[field.name] == item[field.valueField]
            ? "selected"
            : "";
        html += `<option value="${item[field.valueField]}" ${selected}>${item[field.textField]}</option>`;
      }
      html += `</select>`;
      ...
      for (const opt of field.options) {
        const selected =
          editData && editData[field.name] == opt.value ? "selected" : "";
        html += `<option value="${opt.value}" ${selected}>${opt.text}</option>`;
      }
      html += `</select>`;
    } else if (field.type === "textarea") {
      html += `<textarea name="${field.name}" ${field.required ? "required" : ""}>${editData ? editData[field.name] || "" : ""}</textarea>`;
    } else {
      html += `<input type="${field.type}" name="${field.name}" value="${editData ? editData[field.name] || "" : ""}" ${field.required ? "required" : ""} ${field.min !== undefined ? 'min="' + field.min + '"' : ""} ${field.max !== undefined ? 'max="' + field.max + '"' : ""}>`;
    }
    html += `</div>`;
  }
...
async function saveForm() {
        ...
        : currentTable === "departments"
          ...
          : currentTable === "jobs"
            ...
            : currentTable === "projects"
              ? "project"
              : currentTable === "project_records"
                ? "project_record"
                : currentTable === "daily_work_reports"
                  ? "daily_work"
                  : "task"
    ];
  const data = {};
  ...
  const endpoint =
    currentTable === "daily_work" ? "daily_work_reports" : currentTable;
  await apiPost(endpoint, data);
...
async function editItem(type, id) {
  ...
  const item = data.find((d) => d[Object.keys(d)[0]] == id);
  if (item) {
    item.id = id;
    openModal(type, item);
  }
